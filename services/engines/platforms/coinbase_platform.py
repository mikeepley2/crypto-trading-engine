...
class CoinbasePlatform(TradingPlatformInterface):
            ...
            if hasattr(e.response, 'status_code'):
                if e.response.status_code == 429:
                    ...
                    try:
                        ...
                        if 'trading not enabled' in error_details.lower():
                            raise InvalidOrderError(f"Trading not enabled for this trading pair: {error_details}")
                        elif 'permission' in error_details.lower():
                            raise InvalidOrderError(f"Trading permission denied: {error_details}")
                        else:
                            raise PlatformAuthenticationError(f"Access forbidden: {error_msg} - {error_details}")
                    except (ValueError, KeyError):
                        raise PlatformAuthenticationError("Access forbidden - check API permissions")
    ...
    async def place_order(self, order_request: OrderRequest) -> OrderResponse:
        """Place a new order"""
        # Validate order
        is_valid, error_msg = await self.validate_order_request(order_request)
        if not is_valid:
    ...
    async def _check_trading_status(self, symbol: str) -> Tuple[bool, Optional[str]]:
        """Check if trading is enabled for a specific symbol"""
        try:
            ...
            if symbol in known_working_pairs:
                return True, None
            ...
            if not product:
                return False, f"Trading pair {symbol} not found"
            ...
            if status != 'online':
                return False, f"Trading pair {symbol} is not online (status: {status})"
            
            if trading_disabled:
                return False, f"Trading is disabled for {symbol}"
            
            if cancel_only:
                return False, f"Trading pair {symbol} is in cancel-only mode"
            ...
            if symbol in known_working_pairs:
                return True, None
            return False, f"Unable to verify trading status for {symbol}: {str(e)}"

    async def validate_order_request(self, order_request: OrderRequest) -> Tuple[bool, Optional[str]]:
        """Validate order request including trading status"""
        # Basic validation
        if order_request.order_type == OrderType.MARKET:
        ...
        # Check trading status for the symbol
        is_tradable, trading_error = await self._check_trading_status(order_request.symbol)
        if not is_tradable:
            return False, f"Trading not enabled: {trading_error}"
