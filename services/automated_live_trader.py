...
class AutomatedLiveTrader:
    ...
    def get_fresh_recommendations(self):
        """Get only fresh recommendations - TIMEZONE FIXED"""
        try:
            \1from backend.shared.database_connection_manager import db_manager, get_db_connection, get_db_cursor, execute_query
            
            # Connect to database
            db_config = {
                'host': os.getenv('DB_HOST', '192.168.230.163'),
                'user': os.getenv('DB_USER', 'news_collector'),
                'password': os.getenv('DB_PASSWORD', '99Rules!'),
                'database': 'crypto_transactions'
            }
            
            with get_db_cursor('crypto_transactions', dictionary=True) as cursor:
            
            # TIMEZONE FIX: Use database NOW() instead of Python UTC time
            query = """
