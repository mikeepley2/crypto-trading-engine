...
class SignalGenOrchestrator:
    ...
    def setup_database(self):
        ...
        try:
            db_config = {
                'host': os.getenv('MYSQL_HOST', 'host.docker.internal'),  # Use host.docker.internal for Docker to localhost
    ...
    async def get_current_price(self, symbol: str) -> float:
        ...
        try:
            ...
            return float(result['close']) if result else 50000.0
            
        except Exception as e:
            logger.error(f"Error getting current price for {symbol}: {e}")
            return 50000.0  # Mock fallback price
    ...
    async def get_ml_prediction(self, symbol: str, features: Dict[str, float]) -> Dict[str, Any]:
        """Get ML prediction using ML Engine microservice"""
        try:
            request_data = {
                "symbol": symbol,
                "features": features,
    ...
    async def get_market_context(self, symbol: str) -> Dict[str, Any]:
        """Get market context using Market Context microservice"""
        try:
            request_data = {
                "symbol": symbol
            }
            ...
            if result:
                ...
                logger.warning(f"Market context analysis failed for {symbol}")
                return {}
    ...
    def setup_routes(self):
        ...
        async def generate_signal_endpoint(request: SignalGenerationRequest):
            ...
            try:
                ...
                logger.info(f"Request {request_id}: 200 response in {duration_ms:.2f}ms - signal: {result.signal_type}, confidence: {result.confidence}")
